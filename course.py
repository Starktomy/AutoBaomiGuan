...
class CourseManager:
    def __init__(self, session, token, log_callback=None):
    ...
    def get_course_directory(self, course_packet_id, scale=1):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取课程目录失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def get_course_info(self, course_packet_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取课程信息失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def get_course_resources(self, course_packet_id, directory_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取课程资源列表失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def save_study_record(
        ...
        try:
            ...
            self.log(f"{Fore.RED}保存观看记录失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def get_course_progress(self, course_packet_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取课程进度失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def _convert_time_to_seconds(self, time_str):
        try:
            ...
            self.log(f"{Fore.RED}时间格式转换失败: {e}{Style.RESET_ALL}")
            return 0
    ...
    def get_exam_info(self, course_packet_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取考试信息失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def get_exam_answers(self, exam_id, random_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取试卷答案失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def submit_exam_answers(self, exam_id, answers, start_date, random_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}提交考试答案失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def get_exam_result(self, exam_id):
        ...
        try:
            ...
            self.log(f"{Fore.RED}获取考试成绩失败: {e}{Style.RESET_ALL}")
            return None
    ...
    def complete_exam(self, course_packet_id):
        ...
        if not exam_info or not exam_info.get("data"):
            self.log(f"{Fore.RED}获取考试信息失败{Style.RESET_ALL}")
            return False
        self.log(f"{Fore.GREEN}获取考试信息成功{Style.RESET_ALL}")

        exam_id = exam_info["data"][0].get("examId")
        if not exam_id:
            self.log(f"{Fore.RED}未找到考试 ID{Style.RESET_ALL}")
            return False
        ...
        if not exam_paper or not exam_paper.get("data"):
            self.log(f"{Fore.RED}获取试卷答案失败{Style.RESET_ALL}")
            return False
        self.log(f"{Fore.GREEN}获取试卷答案成功{Style.RESET_ALL}")

        random_id = exam_paper["data"].get("randomId")
        if not random_id:
            self.log(f"{Fore.RED}获取 randomId 失败{Style.RESET_ALL}")
            return False
        ...
        if result and result.get("status") == 0:
            self.log(f"{Fore.GREEN}答案提交成功！{Style.RESET_ALL}")
            self.log(f"{Fore.YELLOW}正在查询考试成绩...{Style.RESET_ALL}")
            time.sleep(2)
            ...
            if exam_result and exam_result.get("status") == 0 and exam_result.get("data"):
                ...
                self.log(f"{Fore.GREEN}考试成绩查询成功！{Style.RESET_ALL}")
                self.log(f"{Fore.CYAN}考试名称: {data.get('exam_name', '未知')}{Style.RESET_ALL}")
                self.log(f"{Fore.CYAN}考试成绩: {data.get('score', '未知')} 分{Style.RESET_ALL}")
                self.log(f"{Fore.CYAN}答题次数: {data.get('answerCount', '未知')} 次{Style.RESET_ALL}")
                ...
                self.log(f"{Fore.YELLOW}考试成绩查询失败或暂未生成，使用默认分数更新考试状态{Style.RESET_ALL}")
                self.finish_exam(course_packet_id)
            return True

        self.log(f"{Fore.RED}答案提交失败！{result.get('message', '') if result else ''}{Style.RESET_ALL}")
        return False
    ...
    def study_course(self, course_packet_id):
        ...
        if not directory or not directory.get("data"):
            self.log(f"{Fore.RED}获取课程目录失败{Style.RESET_ALL}")
            return False
        ...
        for section in directory["data"]:
            ...
            for sub in section["subDirectory"]:
                ...
                if not resources or not resources.get("data") or not resources["data"].get("listdata"):
                    self.log(f"{Fore.RED}获取课程资源列表失败{Style.RESET_ALL}")
                    continue
                ...
                for resource in resources["data"]["listdata"]:
                    ...
                    if result and result.get("status") == 0:
                        self.log(f"{Fore.GREEN}完成学习: {resource['name']}{Style.RESET_ALL}")
                    else:
                        self.log(f"{Fore.RED}学习失败: {resource['name']}{Style.RESET_ALL}")
                    time.sleep(2)
    ...
    def finish_exam(self, course_packet_id, exam_score=100):
        ...
        try:
            ...
            if result and result.get("status") == 0:
                self.log(f"{Fore.GREEN}考试状态更新成功！{Style.RESET_ALL}")
                return True
            self.log(f"{Fore.RED}考试状态更新失败！{result.get('message', '')}{Style.RESET_ALL}")
            return False
        except Exception as e:
            self.log(f"{Fore.RED}更新考试状态失败: {e}{Style.RESET_ALL}")
            return False
